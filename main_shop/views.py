...
# Проверяем наличие requests, иначе логируем предупреждение
try:
    import requests
    REQUESTS_AVAILABLE = True
except ImportError:
    REQUESTS_AVAILABLE = False
    print("WARNING: requests library not available. Telegram notifications will be disabled.")
...
def forms(request):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            # Отправляем в Telegram (если requests доступен)
...
Сообщение: {message}"""
                ...
                print("Requests library not available, skipping Telegram notification")
...
Ссылка на товар: http://127.0.0.1:8000/goods/product/{product_id}/"""
                            ...
                            print(f"Ошибка отправки в Telegram: {e}")
                            return HttpResponse("Заказ принят! (Telegram уведомление не отправлено)", status=200)
                    else:
                        return HttpResponse("Telegram настройки не найдены.", status=500)
                else:
                    return HttpResponse("Заказ принят! (Telegram уведомление недоступно)", status=200)
